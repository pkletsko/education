...
var getIntersectionNode = function(headA, headB) {
    ...
    function findIntersection(nodeA, nodeB) {
        // Base case: if one of the nodes is null, return null
        if (!nodeA && !nodeB) return null;

        // If one node reaches the end, switch to the other list's head
        if (!nodeA) return findIntersection(headB, nodeB);
        if (!nodeB) return findIntersection(nodeA, headA);
